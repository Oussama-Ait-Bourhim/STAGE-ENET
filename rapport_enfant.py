...
#importation data
import pandas as pd
df=pd.read_excel("data\carnet_enfant_ENET2012.xlsx")
...
df['Groupe_âge']= df['Groupe_âge'].replace([1,2],['moins 7 ans ','7 à 14 ans'])
...
def dataframe_to_word(data):
    ...
    t.style = 'Table Grid'
    doc.save('./rapport_enfant.docx')


doc.add_heading('Enquête nationale sur l’emploi du temps des marocains 2012', 0)
doc.add_heading('Tableaux statistiques pour les enfants (age -15 ans)', )
doc.add_page_break()
...
doc.add_heading('Tableau 3 : Temps moyen (en minute par jour) de la population âgée de 15 ans et plus selon les activités détaillés, le  niveau scolaire et le sexe', 0)
doc.add_heading('ENSEMBLE', 3)
...
#selon type activité
doc.add_heading('Tableau 4 : Temps moyen (en minute par jour) de la population de âgée 15 ans et plus selon les activités détaillés, le type activité et le sexe', 0)
doc.add_heading('ENSEMBLE', 3)
